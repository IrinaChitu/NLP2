...
class Emotion_Lexicon:
    ...
    def add_association(self, phrase, emotion):
        ...
        ), "Encountered an invalid emotion -- {}".format(emotion)
...
def load_emolex_at_word_level(
    emolex_word_file, emotion_lexicon, positivity_lexicon=None
):
    with open(emolex_word_file) as f:
        all_word_entries = f.read().splitlines()

    for entry in tqdm(
        all_word_entries, desc="processing lexicon entries (word)", ncols=100
    ):
...
def load_emolex_at_sense_level(
    emolex_sense_file, emotion_lexicon, positivity_lexicon=None
):
    with open(emolex_sense_file) as f:
        all_sense_entries = f.read().splitlines()

    for entry in tqdm(
        all_sense_entries, desc="processing lexicon entries (sense)", ncols=100
    ):
...
def build_emotion_lexicon(
    emolex_files_dir="./emotion_analysis/lexicon_processing/nrc_emolex_v0.92",
    show_progress=False,
    lemmatization=True,
):

    emolex_word_file = os.path.join(
        emolex_files_dir, "NRC-Emotion-Lexicon-Wordlevel-v0.92.txt"
    )
    emolex_sense_file = os.path.join(
        emolex_files_dir, "NRC-Emotion-Lexicon-Senselevel-v0.92.txt"
    )

    assert os.path.exists(
        emolex_word_file
    ), "The emolex directory does not contain the lexicon at word level -- {}".format(
        os.path.basename(emolex_word_file)
    )
    assert os.path.exists(
        emolex_sense_file
    ), "The emolex directory does not contain the lexicon at sense level -- {}".format(
        os.path.basename(emolex_sense_file)
    )
...
class Song:
    def __init__(self, csv_row=None, header=None, lyrics=None):
        assert (
            csv_row is not None and header is not None
        ) or lyrics is not None, "Must provide an entry from the dataset csv or provide a string containing only lyrics!"
        self.lemmatizer = WordNetLemmatizer()
        self.data = {}
        if lyrics is None:
...
class Lyrics_Dataset:
    ...
    def get_song(self, artist, title):
        ...
        if not artist_found:
            ...
            return None
        print(
            "found the artist {}, but was not able to find the given song title -- {}".format(
                artist, title
            )
        )
        print(
            "showing {} songs belonging to {} from the dataset...".format(
                len(songs_of_this_artist), artist
            )
        )
...
class Emo_Analysis:
    def __init__(
        self, lexicon_processing_dir=None, load_dataset=False, dataset_csv=None
    ):
    ...
    def analyze_song(
        ...
        if not random:
            assert (
                artist is not None and song_title is not None
            ) or song is not None, "Either provide an artist and a song title or directly provide a song object!"
            if song is None:
        ...
        assert (
            song is not None
        ), "The given artist ({}) or song title ({}) does not exist!".format(
            artist, song_title
        )
        if show_progress:
            print(
                'analyzing "{}", by {}'.format(song.data["song"], song.data["artist"])
            )
        ...
        while seq_idx < len(song_content):
            ...
            for (
                ...
                self.emolex.get_possible_phrase_lengths()
            ):  # the possible phrase lengths are already given in descending order
    ...
    def analyze_artist(self, artist, return_percentages=True, show_progress=True):
        ...
        assert (
            len(songs) > 0
        ), "There were no songs found for the given artist ({})!".format(artist)
        ...
        for song in song_iterator:
            current_emotion_stats = self.analyze_song(
                song=song, return_percentages=False, show_progress=False
            )
            for emotion in self.emolex.valid_emotions:
...
def analyze_new_lyrics(
    ...
    result = {
        k: v
        for k, v in sorted(
            result.items(), key=lambda item: item[1]["percentage"], reverse=True
        )
    }
    return result
...
def analyze_artist(
    ...
    for emo, perc in stats.items():
        result[emo] = {"percentage": round(perc * 100, 2)}
    result = {
        k: v
        for k, v in sorted(
            result.items(), key=lambda item: item[1]["percentage"], reverse=True
